...
from ..models import DimBinFiles, Simulation, SimulationInputFile, SimulationOutputFile


class DimBinFileResource(ModelResource):
    size = fields.IntegerField(readonly=True)

    class Meta:
        queryset = DimBinFiles.objects.all()
        resource_name = 'dim_bin_files'
        excludes = ['content']
    ...
    def dehydrate_version(self, bundle):
        """
        Return the version field as UTC in ISO format 8601 format.  By default, this datetime field is being returned
        in the local timezone.
        """
        file_version = bundle.obj.version
        return file_version.isoformat()

    def dehydrate_size(self, bundle):
        return len(bundle.obj.content)
    ...
    def download_file(self, request, **kwargs):
        ...
        wrapped_content = ContentFile(input_file_django_obj.content)
        ...
        response['Content-Length'] = len(input_file_django_obj.content)
        return response
...
def get_output_file_names(simulation):
    ...
    """
    # For now, we assume it's an OpenMalaria simulation  TODO: use the simulation's model field
    return ('ctsout.txt', 'output.txt', 'model_stdout_stderr.txt', 'BinnedReport.json', 'DemographicsSummary.json', 'InsetChart.json', 'VectorSpeciesReport.json')