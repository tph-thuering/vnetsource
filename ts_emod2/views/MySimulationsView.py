...
class MySimulationsView(TemplateView):
    template_name = "ts_emod2/my_simulations.html"

    def get_context_data(self, **kwargs):
        context = super(MySimulationsView, self).get_context_data(**kwargs)
        dim_user = DimUser.objects.get(username=self.request.user.username)
        context['dim_user'] = dim_user
        context['nav_button'] = 'my_simulations'
        ...
        if type_currently_shown == 'All' or type_currently_shown == 'EMOD':
            emod_scenarios = EMODScenario.objects.filter(deletion_timestamp__isnull=True, user=dim_user).order_by('-id')
        else:
            emod_scenarios = None
