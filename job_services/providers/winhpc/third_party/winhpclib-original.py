...
class Error(Exception):
    ...
    def __str__(self):
        return str(self.status)+" "+self.reason+": "+self.details
...
class Server:
    ...
    def __init__(self, headnode, username, password, port=443):
        ...
        self._base_url = "/WindowsHPC/"+headnode+"/"
        ...
        self._auth_header = {'Authorization' : auth_header_value}
    ...
    def check_successful_response(self, response):
        ...
        if(response.status is not httplib.OK):
            ...
            details = ''            
            xml_details = minidom.parse(response)
            
            if(xml_details.firstChild is not None):
                if(xml_details.firstChild.firstChild is not None):
                    details = xml_details.firstChild.firstChild.data
    ...
    def get_properties(self, resource_url):
        ...
        self._connection.request('GET', self._base_url + resource_url, None, self._auth_header)
